...
namespace Core.Web
{
    public class WebClient
    {
        ...
        [DefaultValue("West Wind HTTP .NET")]
        string cUserAgent;
        ...
        public WebClient(string userAgent) : this (userAgent, null, null) {}
        
        public WebClient(string userAgent, WebAuth authentication) : this(userAgent, authentication, null) {}

        public WebClient(string userAgent, WebAuth authentication, WebProxy proxy)
        {
            this.cUserAgent = userAgent;
            this.oAuthentication = authentication;
            this.oProxy = proxy;
        }
        ...
        public void AddPostFile(string Key, string FileName)
        {
            ...
            if (this.nPostMode != 2)
            {
                ...
                WebException ex = new WebException(cErrorMsg);
                throw ex;
            }
            ...
            catch (Exception e)
            {
                ...
                throw new WebException("File upload failed", e);
            }
        ...
        protected StreamReader GetUrlStream(string Url, HttpWebRequest Request)
        {
            ...
            try
            {
                ...
                if (this.oAuthentication.UserName.Length > 0)
                {
                    if (this.oAuthentication.UserName == "AUTOLOGIN")
                        Request.Credentials = CredentialCache.DefaultCredentials;
                    else
                        Request.Credentials = new NetworkCredential(this.oAuthentication.UserName, this.oAuthentication.Password);
                }
                else
                    Request.Credentials = CredentialCache.DefaultCredentials;

                if (this.oProxy != null) Request.Proxy = oProxy;
        ...
        //Sankar: Replace custom proxy parameters with System.Net.IWebProxy
        //protected void InitializeProxy(HttpWebRequest Request)
        //{
        //    // *** Handle Proxy Server configuration
        //    if (this.oProxy..ProxyAddress.Length > 0)
        //    {
        //        if (this.oProxy.Address == "DEFAULTPROXY")
        //        {
        ...
        //        else
        //        {
        //            System.Net.WebProxy loProxy = new WebProxy(this.cProxyAddress, true);
