...
namespace FireDragan
{
  ...
  class WindowManager
  {
    ...
    public void Close()
    {
        ...
        int closingIndex = _tabControl.SelectedIndex;
      ...
      if (page != null)
      {
        ...
        // Is this the last tab being closing
      if (_tabControl.TabPages.Count == 0)
          _tabControl.Visible = false;
      else
          // set focus to the last before tab
          _tabControl.SelectedIndex = closingIndex - 1;
    ...
    public ExtendedWebBrowser New(bool navigateHome)
    {
      ...
      browserControl.WebBrowser.DocumentCompleted += new WebBrowserDocumentCompletedEventHandler(WebBrowser_DocumentCompleted);
        browserControl.WebBrowser.NewWindow += new System.ComponentModel.CancelEventHandler(WebBrowser_NewWindow);
        browserControl.WebBrowser.StartNavigate += new EventHandler<BrowserExtendedNavigatingEventArgs>(WebBrowser_StartNavigate);
      browserControl.WebBrowser.Quit += new EventHandler(WebBrowser_Quit);
      // Add the new page to the tab control
      _tabControl.TabPages.Add(page);

        if(SettingsHelper.Current.OnOpenNewFocusTab)
    ...
    public void WebBrowser_NewWindow(object sender, System.ComponentModel.CancelEventArgs e)
      {
        if(this._tabControl.TabPages.Count >= SettingsHelper.Current.MaxTabs)
        {
            //SessionManager.Current.Session.AddLink(e.Url.ToString());
            e.Cancel = true;
        }
      }

          public void WebBrowser_StartNavigate(object sender, BrowserExtendedNavigatingEventArgs e)
      {
        if(this._tabControl.TabPages.Count >= SettingsHelper.Current.MaxTabs)
        {
            e.Cancel = false;
